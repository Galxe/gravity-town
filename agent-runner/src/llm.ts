...
export function buildSystemPrompt(
  ...
  const lines = [
    ...
    "The Arena is an OPTIONAL side-game layered on the main world. It does NOT touch your hexes or ore — only your G (Arena currency, separate from ore).",
    ...
    "",
    "Currency: G (funded by owner via faucet). DO NOT spend ore on Arena. Ore is for the main world (build, attack, raid).",
    "Two ways to get cards: (1) arena_buy from SHOP at fixed G price (see arena_list_units), (2) arena_buy_listing from secondary MARKET at seller's price — check arena_list_market for deals.",
    "Cards go to INVENTORY first, then you place them on bench (arena_place_card). Cards on bench can't be listed on market.",
    ...
    "  arena_get_state(agent_id) — see your current bench, ELO, bucket, G balance.",
    "  arena_buy(agent_id, unit_type) — buy card from shop → inventory. Costs G.",
    "  arena_place_card(agent_id, card_id, slot) — inventory → bench slot.",
    "  arena_remove_card(agent_id, slot) — bench → inventory (no refund).",
    "  arena_sell(agent_id, slot) — clear bench slot. Triggers ON_SELL abilities.",
    ...
    "  arena_roll(agent_id) — refresh shop seed. Costs 1 G.",
    ...
    "  - The Arena is OPTIONAL. Hex play, not Arena, decides the main scoreboard.",
    "",
    "=== RULES ===",
...
export function buildUserPrompt(context: AgentContext): string {
  ...
  if (arena) {
    ...
    arenaPrompt = [
      ...
      `Bench: ${benchSummary}`,
      filledSlots === 0
        ? "You have NO units yet. If you have G (>= 3), consider arena_list_units → arena_buy → arena_place_card → arena_submit to enter the Arena. (Optional.)"

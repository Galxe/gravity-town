...
- **「改造」只会**额外**叠上一笔迁移税，不会省事：** 改 `GTreasury` 的发奖/escrow/提现权限 + 连带修被它弄坏的 `CardLedger`/`ArenaEngine`、把旧地图快照导进新世界、给共享 `Router` 上 dual-read 兼容层、三服务 feature-flag 软切……这些**全是 greenfield 不需要的**。

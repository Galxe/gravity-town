...
**押对赢什么**：练手的「数学题盘」押对**只发 AP**（人人答对则退还本金、没有彩池）；有真对手的「地图盘」押对**额外赢回彩池**（输家押进来的 G）。**AP 永远只能赢、买不到。**

### A. 建什么（文件一眼看完）

```
contracts/src/v2/
  V2Treasury.sol               钱(G)的金库：押注托管 / 按市场指令放奖金 / 提现   [步1]
  PredictionMarketEngine.sol   开盘 / 押注 / 关盘 / 发 AP（含题目模板）           [步2,5]
  MarketSettlementResolver.sol 判一个盘的对错（数学题自己算 / 地图盘读战斗记录）   [步2,5]
...
| 步 | 做什么 | 怎么算做完（验收） | 约需 |
|----|--------|--------------------|----|
| 0 | **定接口**：7 个合约的接口 + 假实现(mock)先写好、封板（=接口定稿后锁死、开工期间不改） | 接口写齐并锁死，各步能照着它**同时**开工 | 1 周 |
| 1 | **金库** `V2Treasury` | 玩家能把 G 充进金库（入金；测试期发币）、押 20G 能锁住、结算时把托管的钱正确放出（赢家拿走 / 无对手退款）、协议**提不走**押注的钱 | ~2 周 |
...
- **谁放钱 vs 谁下令**：金库(`V2Treasury`)只**保管和放钱**，**唯一能下令发奖的是「市场」**(`PredictionMarketEngine`)。
- **4 条硬底线（钱 + 地图）**：① AP 只能从赢盘来 ② 发奖只有「市场」一个入口（市场下令、金库放钱）③ 押注的钱锁住、协议提不走 ④ 防御有上限、被打下来清零。（这几条都有**自动检查守着、每步都跑、没过就不让合并**，完整一组见后面 §8。）

### E. 还没定的（都不挡开工）

- 一堆**数值**（伤害、各种上限、手续费率、随机结算窗口…）等 owner 拍——先用占位值开发,接口已留好（清单见附录 §12）。
- **外部事件盘**（押链外真事,如美股/体育）——最后做,要先定信任模型。
- **卡牌**（攻防小加成）——第一个版本之后再做。

---

> 以下是**给并行拆活的人**看的详细执行计划（lane / 依赖 / 排期 / 风险 / 数值表）。只想一眼看懂、读上面「总览」即可。
...
> ### ✅ 架构已定稿（无 GATE 0）
> **旧稿的「13 项 M0 决策签字门」已删除**——那些项已 research 定稿为 §3 已定架构（World 自持、future-blockhash RNG、全新 `V2Treasury`、QuestionRegistry 内嵌、全新 `V2Router`…）。**接口（§4）即刻可冻、lane 即刻可并行**，不再等任何签字。仅剩 §12 数值与 M7 ORACLE 信任模型待 owner，均**不阻塞**冻接口（§3.5）。
...
- **6 个必须先落地的执行工件**（详见 §1.工件清单）：`/contracts/src/interfaces/` MVP 接口（§4 清单，对应 §3.4 共 7 个新合约含 `V2Treasury`/`V2Router`）+ `HexGrid.sol` + `Mock*`、`/contracts/test/WorldInvariants.t.sol`、`/lane-PLAN.md`、`/docs/gates.md`、`/docs/§12-values.md`、全新 `V2Router` + setter。无这 6 件，本文只是路线图、不是可执行部署规格。
...
| # | 工件路径 | 内容 | 谁建/何时 |
|---|---|---|---|
| 1 | `/contracts/src/interfaces/` + `/contracts/src/libraries/HexGrid.sol` + `Mock*` | MVP `I*.sol`（§4 清单，对应 §3.4 共 7 个新合约；QuestionRegistry 内嵌、不单列）+ `HexGrid` library + 3 个确定性 Mock | lead，冻结后 3 天内 |
...
| 6 | 全新 `V2Router.sol`（旧 Router 不碰） | v2 地址槽 + 公共 getter + setter（§3.4 / §4 Router 段） | lead，随接口冻结 |
...
```
                         ┌─────────────────────────────┐
                         │  §3 已定架构 (research 定稿)  │   ← World=B / RNG=future-blockhash
...
- **`V2World` schema（定稿）**：`hexOwner`=`agentId`(uint256，`0`=neutral，无碰撞——agentId 从 1 起 `AgentRegistry.sol:61/:81`)；`hexDefense`（上限/衰减/**capture 清零**，`game-design.md:426/488`）；spawn/respawn allocator（复用 `_findEmptyCluster` 螺旋 `GameEngine.sol:1089`，但**必须保证 0-hex agent 永能再落脚**——保留 neutral/respawn 池或 capped 回场赛）；`checkpoints` mapping 按 `bytes32 eventId`；`v2Score(agentId)`（`game-design.md:533-539`，**花掉的 AP 不计分**）。数值（`D_max/halflife/p_floor/SCORE_PER_*`）→ §12。
...
### 3.2 财库与经济（greenfield：新建 `V2Treasury`，不碰 `GTreasury`）
...
- **backing 来源/顺序**：losing stakes → 预存补贴池 → 已拨入 surplus；creditG 前必有等额具名 backing（顺序逻辑在 `PredictionMarketEngine` 结算里）。比例/补贴释放条件 → §12/owner。
...
`/contracts/src/interfaces/`（net-new MVP I*.sol，§3.4 共 7 个新合约；QuestionRegistry 内嵌 PME、不单列；`V2Treasury`/`V2Router` 为全新合约）：
...
- `IRNGProvider.sol`（future-blockhash，§3.1）
- `IV2Treasury.sol`（**全新合约**，不碰旧 GTreasury）：`onlyMarket creditG` / `reservedBackingG` / `surplusG` / `netTreasuryTakeG` cap / `SECONDARY_CARD_TRANSFER` / 多签提现（§3.2）
...
- **APLedger**（§5）：`apBalance`、`earnedInEpoch`、`ownerTrivialEarnedInEpoch`、`globalTrivialEarnedInEpoch`、`creditFromMarket(onlyMarket)`、`spendForAction(onlyWorld)`（方案 B：唯一调用者 = V2World）。
...
| 🔴 L-C 世界+Hex | ☐ 主 / ☐ 备 | ~2.5w | §3.1 已定（World 自持；fresh 世界、不导入旧快照） |
...
- **🔴 L-C 世界+Hex** — 模块：`V2World`、`HexGrid` lib、spawn/respawn allocator、`hexDefense`、checkpoint 写入 + finalized gate、`v2Score`（greenfield：fresh 空地图开局，**无旧世界快照导入**）。交付：v2 hex 存储 + 邻接库 + 防御 + **checkpoint mapping（含 `IWorldCheckpoint` 全字段）** + score；**C1 = checkpoint event/mapping schema + `IWorldCheckpoint` 须最先交付并 M2 中旬冻结（见 §10 子里程碑 M2.a）**。并行安全：✅ 与 L-A/L-B 完全并行。
...
- **Router 配合**：三服务配置直接指向**全新 `V2Router`**（旧 Router 不碰、继续服务 v1；无 dual-read，§4 Router 段）。
...
- **lead（接口 owner）+ ☐ lead-备（deputy，须填真实名）**：采纳架构(§3)、主笔接口冻结、维护**全新 `V2Router`** setter、守三个 join 点、L-C 改 World 接口时同步 mock。接口冻结**定稿权归 lead 一人**，但 deputy 共担 V2Router / mock-sync / join 仲裁，避免单点。`/contracts/src/interfaces/`、`HexGrid.sol`、`V2Router.sol` 写入 `CODEOWNERS`。**⚠️ lead 是关键路径上最集中的单点**（架构采纳、唯一「并行总开关」冻结、Router、三 join 全过 lead）——故 (1) 必须配 deputy；(2) 接口变更周转 SLA ≤ 24h（超时 escalate）；(3) §10 PERT 须把 lead 的冻结/join/mock-sync 当**共享资源**计，不可假设 lead 同时还全职背一条 lane。
...
11. **M5.5 testnet 部署 + 市场/ops keeper 接入 + 性能调优（~2w）→ M5.7 mainnet 部署**（注：**RNG-resolve keeper 是 MVP 承重、随 real RNG 在 M4 落地**，不在此 M5.5 列，见 §5 L-D / §11）
12. `Post-MVP：L-F → L-O`
...
| 风险 | 影响 | 缓解 |
|---|---|---|
| **架构决策被某 lane 私自改走老路** | §3 已定架构（World=B / RNG=future-blockhash / 财库权限 / Router）被绕过，破坏「干净」前提 | 改 §3 架构须走 ADR（§9）；CI gate (a)：改架构无 ADR → block；`interfaces/`+`Router.sol`+财库权限模型只 lead 可改（CODEOWNERS） |
| **v2 合约 + V2Router 全 0 实现** | lane day-1 找不到 `IAPLedger`/`IV2Treasury` 从哪 import；切换时 V2Router 槽没填 → 三服务找不到 v2 地址 | 冻结期 lead 头 3 天交 `/contracts/src/interfaces/I*.sol`+`HexGrid.sol`+`Mock*`+**全新 `V2Router`(槽+setter)**，不等 M1 实现（§4）；旧 Router 不碰，无 dual-read 0-day 风险 |
...
| **stub→real 切换语义** | dev stub → 真实 v2 切换时哪些状态保留/重置不明 | dev 期用 stub 数据；切到真实 v2 = 部署 + 三服务指向 `V2Router`；**有可执行 DoD（§6 join③ 第 4 门：v2 合约部署 + V2Router 槽填好 + 三服务指向一致 + 闭环冒烟 + 可回滚指回 v1，L-I+L-T 双签）** |
| **结算执行者归属 + keeper 存活/超时** | P1/P2 demo 需盘能结算，但 keeper 排 M5.5（demo 之后）、无 lane 拥有；且题到期 keeper 离线则市场卡死 | **市场 `settle()` permissionless**（owner=L-B），demo 期 agent-runner 兜底，超时 `settleDeadline`→void+refund（**市场结果非攻击者可控，退款安全**）；**RNG-resolve keeper 是另一回事——MVP 承重、`expire` 没收 AP 不退（§3.1），勿混**；测 keeper 失效路径（§5/§7/§10） |

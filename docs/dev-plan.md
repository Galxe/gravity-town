...
> 配套：架构权威 [`docs/architecture.md`](architecture.md)（greenfield 系统架构）· 工程稿 [`docs/game-design.md`](game-design.md)（合约/结构/不变量）· 玩家视角 [`docs/game-design-ux.md`](game-design-ux.md)。本文**不重写合约规格**，只把里程碑转成可并行的 lane / 依赖 / 节奏。**字段 / 公式 / 不变量以 `game-design.md` 为准；凡涉 Treasury / World / Router / 迁移，一律以 `architecture.md` 的 greenfield 决策为准**（`game-design.md` 那部分已过时、§3 已据 architecture 同步）。不走 PR#76（`feat/ux-demo-roadmap`）已废弃的「万物皆答题」方向。真正要挡在 v2 lane 之外的是**旧 v1 GameEngine 的范畴**（ore/build/arsenal/happiness/raid）——只能作 v1 参考、不可作 v2 实现（见 §8 v1 旧 scope 越界扫描）。

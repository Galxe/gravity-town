...
**一句话**: 你和 AI 对手抢一张六边形地图——但要扩张，你不能靠刷资源，**必须先赢一场预测**。每一步都是抉择：现在就花掉手里的行动力，还是押一注去赚更多行动力？

**这是什么产品**: 一个链上领土争夺游戏，对手是 LLM 驱动的 AI agent（人也可以亲自下场）。和所有同类游戏最大的不同——**进攻和防守需要的“行动力”买不到、刷不出，唯一来源是赢一场可验证的预测市场**：你拿游戏币 G 去押“1+1=2”这种机器能判的题，或“2 小时后 hex#42 还归 Agent B 吗”这种游戏内事件。押对 → 拿到行动点 AP → 花 AP 抢邻接地块或加固自己的地块 → 地图变了 → 新局面又能开成新的预测盘。这是一个闭环。
...
**Alice 的第一局**（开局 10 分钟长什么样）: Alice 带着 100 G 进场。她看到一个盘：“2 小时后 hex#42 还归 Bob 吗？”当前赔率 2:1 偏向 Bob。她押 20 G 赌“不会”。赢了 → +5 AP，她现在有了足够的 AP 去打 Bob 旁边的地块；输了 → 她还剩 130 G，换下一盘再试。**这就是整个循环。**

**为什么新 / 为什么有意思**: 它把常见的“刷币 → 买战力”反过来做成“**赢预测 → 得战力 → 打地图**”。这一条改动同时做到三件事——(1) 逼玩家真正去和市场博弈，而不是无脑刷资源；(2) 制造“现在打还是先押一注”的持续抉择，策略张力来自这里；(3) 钱多的人也买不到战力，挡住 pay-to-win。AP 没有商店、不能用钱买；卡牌只是后续的攻防加成道具、由协议利润出资，同样不是 pay-to-win。

**你是谁**: 你只控制**一个** agent，绑定你的链上钱包，你所有的钱（G）和战力（AP）都归这一个 agent。你不能同时跑多个 agent——后文提到的 sybil / 限流，正是为了防止**一个人**开一堆假 agent 自己跟自己对赌来刷战力。

**怎么赢**: 赛季结束时 `v2Score` 最高者胜——分数来自你占的地块数、夺地次数、战斗胜场、赢的非 trivial 预测盘数（详见 §6 末）。

> 下面 §1-§2 讲系统骨架，§3-§4 讲**防作弊的经济护栏**（正常游玩时你不会感知到“backing/金库/抽水”这些词，它们是给审计和开发看的，解释这些规则**为什么**存在），§5-§10 讲实现与里程碑。
...
每层一句话职责（自下而上读：市场是战力的唯一来源，行动层把战力变成地图变化）：

```text
Layer 4  NFT 卡牌市场        [攻防加成/出处道具, 非 pay-to-win] 公库出资 + 冻 G 铸造(成就铸稀有卡 / 白板自铸), 复用 Arena/CardLedger, Post-MVP 接入
Layer 3  Action 行动层       [AP 在这里花掉, 改写地图] 花 AP 攻击邻接 hex、给己方 hex 加防御, 或让 0-hex agent 回场
Layer 2  Gambling/AP 层      [AP 的唯一来源] 赢市场获得 AP, 可能同时获得 G 奖励
Layer 1  Market 市场层       [整个经济的发动机] 通用“可验证结局”预测市场, 三类盘 MATH + STATE-by-checkpoint + ORACLE, 按可判定性分批上线(MATH/STATE 先, ORACLE 最后)
```
...
> 闭环何时完整: “地图变化 → 开成新市场”这一段靠 `STATE` 盘闭合，而 `STATE` 盘依赖 M2 的 `V2World` checkpoint。所以 M1 内测只有 `MATH` 盘（押数学题攒 AP），完整闭环要到 `STATE` 上线后才转起来（§9 已写明 MVP 可降级为 `MATH`-only）。

## 3. 两币模型与铁律不变量

> **读前定位**: 玩家只需记住两件事——**G 是你押注和领奖的钱，AP 是你赢盘换来、只能用来行动的燃料**。本节其余内容（backing、公库、抽水、burn、桶隔离）是**防作弊护栏**：保证“赢家一定拿得到钱、G 不通胀、协议不能偷偷抽干玩家资金”。正常游玩时你不会感知到这些词，它们写在这里是给实现者和审计看的。**这些是承重不变量，不能删改。**
